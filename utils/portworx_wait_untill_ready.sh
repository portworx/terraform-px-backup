...
RETRIES=0
PODS_COUNT=$(k get pods -l name=portworx -n kube-system | wc -l | xargs)
while [ "$RETRIES" -le "$LIMIT" ]; do
    READY_COUNT=$(k get pods -l name=portworx -n kube-system -o json | jq -r '.items[] | select(.status.conditions[].type=="Ready") | .metadata.name' | wc -l | xargs)
